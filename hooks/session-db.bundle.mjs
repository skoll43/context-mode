...
`)}function Oe(n){return n.ignoredEnvVar&&n.ignoredReason==="empty"?`Ignored empty ${n.ignoredEnvVar}; using adapter default.`:null}function J(){return`Set ${l} to a writable absolute path.`}function Ae(n){if(!n||typeof n!="object")return null;let e=n.path;return typeof e=="string"&&e.length>0?e:null}var _;function E(n){let e=n.replace(/\\/g,"/");return/^\/+$/.test(e)?"/":/^[A-Za-z]:\/+$/.test(e)?`${e.slice(0,2)}/`:e.replace(/\/+$/,"")}function W(n){let e=n;try{e=Re.native(n)}catch{}let t=E(e);return process.platform==="win32"||process.platform==="darwin"?t.toLowerCase():t}function Z(n,e){return Se("git",["-C",n,...e],{encoding:"utf-8",timeout:2e3,stdio:["ignore","pipe","ignore"]}).trim()}function we(n){let e=Z(n,["rev-parse","--show-toplevel"]);return e.length>0?E(e):null}function Ie(n){let e=Z(n,["worktree","list","--porcelain"]).split(/\r?\n/).find(t=>t.startsWith("worktree "))?.replace("worktree ","")?.trim();return e?E(e):null}function xe(n=process.cwd()){let e=process.env.CONTEXT_MODE_SESSION_SUFFIX;if(_&&_.projectDir===n&&_.envSuffix===e)return _.suffix;let t="";if(e!==void 0)t=e?`__${e}`:"";else try{let r=we(n),s=Ie(n);if(r&&s){let a=W(r),i=W(s);a!==i&&(t=`__${f("sha256").update(a).digest("hex").slice(0,8)}`)}}catch{}return _={projectDir:n,envSuffix:e,suffix:t},t}function et(){_=void 0}function ee(n){return f("sha256").update(E(n)).digest("hex").slice(0,16)}function te(n){let e=E(n),t=process.platform==="darwin"||process.platform==="win32"?e.toLowerCase():e;return f("sha256").update(t).digest("hex").slice(0,16)}function tt(n){let{projectDir:e,contentDir:t}=n,r=te(e),s=g(t,`${r}.db`);if(L(s))return s;let a=ee(e);if(a===r)return s;let i=g(t,`${a}.db`);if(L(i))try{x(i,s);for(let c of["-wal","-shm"])try{x(i+c,s+c)}catch{}}catch{}return s}function nt(n){return Me({...n,ext:".db"})}function Me(n){let{projectDir:e,sessionsDir:t,ext:r}=n,s=n.suffix??xe(e),a=te(e),i=g(t,`${a}${s}${r}`);if(L(i))return i;let c=ee(e);if(c===a)return i;let d=g(t,`${c}${s}${r}`);if(L(d))try{x(d,i)}catch{}return i}var V=1e3,X=5;function b(n){let e=Number(n);return!Number.isFinite(e)||e<=0?0:Math.floor(e)}var o={insertEvent:"insertEvent",getEvents:"getEvents",getEventsByType:"getEventsByType",getEventsByPriority:"getEventsByPriority",getEventsByTypeAndPriority:"getEventsByTypeAndPriority",getEventCount:"getEventCount",getLatestAttributedProject:"getLatestAttributedProject",checkDuplicate:"checkDuplicate",evictLowestPriority:"evictLowestPriority",updateMetaLastEvent:"updateMetaLastEvent",ensureSession:"ensureSession",getSessionStats:"getSessionStats",getSessionRollup:"getSessionRollup",getMaxFileEdits:"getMaxFileEdits",getLatestCommitMessage:"getLatestCommitMessage",incrementCompactCount:"incrementCompactCount",getUsageCursor:"getUsageCursor",setUsageCursor:"setUsageCursor",upsertResume:"upsertResume",getResume:"getResume",markResumeConsumed:"markResumeConsumed",claimLatestUnconsumedResume:"claimLatestUnconsumedResume",deleteEvents:"deleteEvents",deleteMeta:"deleteMeta",deleteResume:"deleteResume",getOldSessions:"getOldSessions",searchEvents:"searchEvents",incrementToolCall:"incrementToolCall",getToolCallTotals:"getToolCallTotals",getToolCallByTool:"getToolCallByTool",getEventBytesSummary:"getEventBytesSummary"},Ue=[["project_dir","TEXT NOT NULL DEFAULT ''"],["attribution_source","TEXT NOT NULL DEFAULT 'unknown'"],["attribution_confidence","REAL NOT NULL DEFAULT 0"],["bytes_avoided","INTEGER NOT NULL DEFAULT 0"],["bytes_returned","INTEGER NOT NULL DEFAULT 0"]];function ne(n){let e=n.pragma("table_xinfo(session_events)"),t=new Set(e.map(s=>s.name)),r=!1;for(let[s,a]of Ue)t.has(s)||(n.exec(`ALTER TABLE session_events ADD COLUMN ${s} ${a}`),r=!0);return r&&n.exec("CREATE INDEX IF NOT EXISTS idx_session_events_project ON session_events(session_id, project_dir)"),r}function rt(n,e){let t=null;try{t=new e(n),ne(t)}catch{}finally{try{t?.close()}catch{}}}var $=class extends v{constructor(e){super(e?.dbPath??B("session"))}stmt(e){return this.stmts.get(e)}initSchema(){try{let t=this.db.pragma("table_xinfo(session_events)").find(r=>r.name==="data_hash");t&&t.hidden!==0&&this.db.exec("DROP TABLE session_events")}catch{}this.db.exec(`
    ...
    `).get(e)??null}getLatestSessionDecision(e){return this.db.prepare(`
      SELECT e.id, e.data, e.category FROM session_events e
      WHERE e.session_id = ? 
        AND e.category IN ('decision', 'plan', 'goal')
      ORDER BY e.id DESC LIMIT 1
    `).get(e)??null}getLatestSessionConstraint(e){return this.db.prepare(`
      SELECT e.id, e.data FROM session_events e
      WHERE e.session_id = ? 
        AND e.category = 'constraint'
      ORDER BY e.id DESC LIMIT 1
    `).get(e)??null}getLatestSubagentTask(e){return this.db.prepare(`
      SELECT e.id, e.data, e.type FROM session_events e
      WHERE e.session_id = ? 
        AND (e.category IN ('subagent', 'task') OR e.type IN ('subagent_launched', 'task'))
      ORDER BY e.id DESC LIMIT 1
    `).get(e)??null}getLatestImplementIntent(e){return this.db.prepare(`
      SELECT e.id, e.data FROM session_events e
      WHERE e.session_id = ? 
        AND e.category = 'intent'
        AND e.data = 'implement'
      ORDER BY e.id DESC LIMIT 1
    `).get(e)??null}getLatestIterationLoop(e){return this.db.prepare(`
      SELECT e.id, e.data FROM session_events e
      WHERE e.session_id = ? 
        AND e.category = 'iteration-loop'
      ORDER BY e.id DESC LIMIT 1
    `).get(e)??null}};export{$ as SessionDB,T as StorageDirectoryError,et as _resetWorktreeSuffixCacheForTests,ne as applyMissingSessionEventsColumns,Je as clearStorageDirectoryCheckCacheForTests,Qe as describeStorageDirectorySource,rt as ensureSessionEventsSchema,Ze as ensureWritableStorageDir,ze as formatStorageDirectoryError,xe as getWorktreeSuffix,te as hashProjectDirCanonical,ee as hashProjectDirLegacy,E as normalizeWorktreePath,Ge as resolveContentStorageDir,tt as resolveContentStorePath,Ye as resolveDefaultSessionDir,nt as resolveSessionDbPath,Me as resolveSessionPath,Q as resolveSessionStorageDir,Ke as resolveStatsStorageDir};
